...
template <class K, class V>
K &xMap<K, V>::getKey(V value)
{
    for (int i = 0; i < this->capacity; i++)
    {
        DLinkedList<Entry *> &temp = this->table[i];
        for (auto pEntry : temp)
        {
            if ( this->valueEQ(pEntry->value, value) )
            {
                return pEntry->key;
            }
        }
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V))
{
    ...
    for (auto pEntry : list)
    {
        if ( this->keyEQ(pEntry->key, key) && this->valueEQ(pEntry->value, value))
        {
            res = pEntry->value;
            if (deleteKeyInMap != nullptr)
            {
