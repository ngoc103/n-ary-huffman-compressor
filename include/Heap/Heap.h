...
using namespace std;
 ...
 void Heap<T>::remove(T item, void (*removeItemData)(T)) {
   ...
   this->count--;
   int parent = (idx - 1)/2;
   if (!aLTb(elements[parent], elements[idx]))
   {
      reheapUp(idx);
   }
   else reheapDown(idx);
...
void Heap<T>:: heapsort(XArrayList<T>& arrayList){
 ...
 void Heap<T>::reheapDown(int position) {
   ...
   while (true)
   {
     ...
     if (left < this->count && !aLTb(elements[largest], elements[left])) 
     {
      largest = left;
     }
     if (right < this->count && !aLTb(elements[largest], elements[right])) 
     {
      largest = right;
     }
 ...
 void Heap<T>::removeInternalData() {
  ...
  {
    this->deleteUserData(this);  // clear users's data if they want
  }
  delete[] elements;
   this->count = 0;
