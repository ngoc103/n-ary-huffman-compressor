...
XArrayList<T>::XArrayList(
    ...
    bool (*itemEqual)(T &, T &),
    int capacity)
    :deleteUserData(deleteUserData), itemEqual(itemEqual),
    count(count = 0), data(data = 0), capacity(capacity = 10) {};
...
void XArrayList<T>::add(int index, T e)
{
    ...
    else if (this->count == this->capacity && this->data != NULL)
    {
        T* temp = this->data;
...
T &XArrayList<T>::get(int index)
{
    ...
    else if (index == 0 && this->empty() == true)
    {
        throw std::out_of_range("Index is out of range!");
    }
    else
    {
        T& ref = this->data[index];
        return ref;
    }
}
...
void XArrayList<T>::ensureCapacity(int index)
{
    ...
    {
        int change = this->capacity;
        if (this->capacity == 0)
        {
            change = 1;
            change = this->capacity * 2 + 1;
        }
        else change = this->capacity * 2 + 1;

        try
        {
            T* temp = this->data;
            this->data = new T[change];
            for (int i = 0; i < this->count; i++)
            {
                this->data[i] = temp[i];
            }
